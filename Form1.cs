...
namespace REGOVID
{
    partial class Form1 : Form
    {
        ...
        private const char separator = (char)59;   // divides customized container into human-readable view(main) that is consisting of GUI input fields
        ...
        public Form1()
        {
            dtBase = $@"Provider=Microsoft.ACE.OLEDB.12.0;Data Source='{Environment.GetEnvironmentVariable(string.Empty + (char)0x0050 + (char)0x0072 + (char)0x006F + (char)0x0067 + (char)0x0072 + (char)0x0061 + (char)0x006D +
                        ...
                        (char)0x0074 + (char)0x0042 + (char)0x0061 + (char)0x0073 + (char)0x0065 + (char)0x002E + (char)0x0078 + (char)0x0061 + (char)0x006C + (char)0x0073 + (char)0x0062}';Extended Properties='Excel 12.0;HDR=Yes'";
            hint = SetFieldColour;
            hint += SetFieldsText;
                ...
                case "Записать":
                    switch (Handler((byte)ops.Action.Записать, out byte sheet, condition, senders.Slf, senders.BirthYear, senders.HomeAddr, senders.Company, senders.Occupation, senders.NameVaccine, senders.Serial, senders.NumbAppoint, senders.DateActual, senders.DatePlan))
                    {
                        case true:
                            switch (sheet)
                            {
                                case 0x00:   // concerning Пациент sheet
                                    ...
                                    switch (string.IsNullOrEmpty(container.Peek().ToString()))
                                    {
                                        ...
                                        default:
                                            ...
                                            for (var i = ExecSQLSingle(patient.GetNext); container.Count > byte.MinValue;)   // on init: increase MAX(IDN) by one
                                            {
                                                reg.Record.UN = container.Peek();
                                                container.Dequeue();
                                                dataGridView1.DataSource = ExecSQLtoSet(senders[ops.Action.Поиск, 1, vaccine].ToString()).DefaultView;   // SQL result. Fetch Вакцина records by UNN
                                                               ...
                                                               (char)0x0020 + (char)0x043E + (char)0x0020 + (char)0x0432 + (char)0x0430 + (char)0x043A + (char)0x0446 + (char)0x0438 + (char)0x043D + (char)0x0435 + (char)0x003F) == string.Empty + (char)0x0059 + (char)0x0065 + (char)0x0073)
                                                {
                                                    case true:
                                                        ...
                                                        break;
                                                    default:   // initialize not familiar IDN but increased by one
                                                        reg.Record.UN = $"{0x00}{i}";   // IDN column might have no numbers therefore recording from this point the "Формат ячеек" of cell above should be "Числовой" in Excel
                                                        break;
                                                }
                                            }
                                ...
                                case 0x05:   // concerning Вакцина sheet
                                    ...
                                    switch (string.IsNullOrEmpty(container.Peek().ToString()))
                                    {
                                        ...
                                        default:
                                            SetGUI(default, default);   // prepare for chance to output SQL result
                                            for (var i = ExecSQLSingle(vaccine.GetNext); container.Count > byte.MinValue;)   // on init: increase MAX(UNN) by one
                                            {
                                                reg.Record.UN = container.Peek();
                                                container.Dequeue();
                                                dataGridView1.DataSource = ExecSQLtoSet(senders[ops.Action.Поиск, 1, patient].ToString()).DefaultView;   // SQL result. Fetch Пациент records by IDN
                                                               ...
                                                               (char)0x0438 + (char)0x0435 + (char)0x043D + (char)0x0442 + (char)0x0430 + (char)0x003F) == string.Empty + (char)0x0059 + (char)0x0065 + (char)0x0073)
                                                {
                                                    case true:
                                                        container.Clear();   // interrupt the read-next
                                                        break;
                                                    default:   // initialize not familiar UNN but increased by one
                                                        reg.Record.UN = $"{0x00}{i}";   // UNN column might have no numbers therefore recording from this point the "Формат ячеек" of cell above should be "Числовой" in Excel
                                                        break;
                                                }
                                            }
                                            SetGUI(true, ops.Action.Записать);
                            ...
                            SetStatement(GetMsg(string.Empty + (char)0x0417 + (char)0x0430 + (char)0x043F + (char)0x0438 + (char)0x0441 + (char)0x044C + (char)0x0020 + (char)0x0443 + (char)0x0441 + (char)0x043F + (char)0x0435 + (char)0x0448 +
                            ...
                            return;
                        default:
                            SetStatement(GetMsg(string.Empty + (char)0x0412 + (char)0x043E + (char)0x0437 + (char)0x043D + (char)0x0438 + (char)0x043A + (char)0x043B + (char)0x0438 + (char)0x0020 + (char)0x0442 + (char)0x0440 + (char)0x0443 +
                        ...
                        case true:
                            switch (sheet)
                            {
                                ...
                                case 0x05:   // concerning Вакцина sheet
                                    ...
                                    break;
                                default:
                                    dataGridView1.DataSource = ExecSQLtoSet(senders[ops.Action.Поиск, 0x02, patient].ToString()).DefaultView;
                                    break;
                            }
                            break;
                    }
                ...
                case "Изменить":   // handle only complete record(e.g. IDN matches UNN)
                    switch (Handler((byte)ops.Action.Изменить, out _, condition, senders.Slf))
                    {
                        case true:
                            ...
                            ExecSQLtoSet(patient.FindBySlf, out Queue tmpContainer);   // seek by input - ФИО GUI field
                            for (timer1.Start(); tmpContainer.Count > (uint)default; hintSign = default)   // on iteration: reset sign of hint for those GUI input fields to its default
                            {
                                reg.Record.UN = tmpContainer.Dequeue();   // take one off
                                var store = ExecSQLtoSet(senders[ops.Action.Поиск, default, patient].ToString());   // fetch a complete record by unique number
                                foreach (DataRow element in store.Rows)
                                    switch (element.Table.Columns.Count)
                                    {
                                        case 0x0A:
                                            for (var i = (byte)default; i < element.Table.Columns.Count; i++)   // on init: index which has relation is one-to-many
                                                           ...
                                                           (char)0x003F) == string.Empty + (char)0x0059 + (char)0x0065 + (char)0x0073)
                                            {
                                                case true:
                                                    ...
                                                    for (var fieldInd = (byte)default; fieldInd < (byte)name.Field.ПлановаяДата; fieldInd++)   // on init: counter for GUI index fields
                                                    {
                                                        for (var capacityFieldInd = (byte)(custContainer[default].Split(separator).Length - 0x01); capacityFieldInd > (byte)default; capacityFieldInd--)   // on init: amount of wanted GUI index fields
                                                    ...
                                                    SetFieldColour(SystemColors.Window);   // restore colour of those GUI input fields
                                                    CLS();   // reset GUI input(by user|fetched) fields
                                                    switch (GetMsg(string.Empty + (char)0x0417 + (char)0x0430 + (char)0x043F + (char)0x0438 + (char)0x0441 + (char)0x044C + (char)0x0020 + (char)0x0443 + (char)0x0441 + (char)0x043F +
                                                                   ...
                                                                   (char)0x043B + (char)0x0436 + (char)0x0438 + (char)0x0442 + (char)0x044C + (char)0x003F) == string.Empty + (char)0x004E + (char)0x006F)
                                                    {
                                                        case true:
                                                            SetGUI(default, default);
                                                            return;   // finish iterating element up here due to foreach-loop
                                                        default:
                                                            for (var i = (byte)default; i < element.Table.Columns.Count; i++)
                                                                SetFieldsText(i, senders[(name.Field)(i + 0x01)]);   // restore text of those GUI input fields that user had entered
                                                            return;   // finish iterating element up here due to foreach-loop
                                                    }
                                                default:   // decline
                            ...
                            {
                                case true:
                                    switch(GetMsg(string.Empty + (char)0x041D + (char)0x0438 + (char)0x0447 + (char)0x0435 + (char)0x0433 + (char)0x043E + (char)0x0020 + (char)0x043D + (char)0x0435 + (char)0x0020 + (char)0x043D +
                                                  ...
                                                  (char)0x043E + (char)0x043B + (char)0x0436 + (char)0x0438 + (char)0x0442 + (char)0x044C + (char)0x003F) == string.Empty + (char)0x004E + (char)0x006F)
                                    {
                                        case true:
                                            SetGUI(default, default);
                                            return;
                                    ...
                                    break;
                                default:
                                    switch (GetMsg(string.Empty + (char)0x0417 + (char)0x0430 + (char)0x043F + (char)0x0438 + (char)0x0441 + (char)0x044C + (char)0x0020 + (char)0x0443 + (char)0x0441 + (char)0x043F + (char)0x0435 +
                                                   ...
                                                   (char)0x0442 + (char)0x044C + (char)0x003F) == string.Empty + (char)0x004E + (char)0x006F)
                                    {
                                        case true:
                                            SetGUI(default, default);
                                            return;
                            ...
                            for (var i = (byte)default; i < 0x0A; i++)
                                SetFieldsText(i, senders[(name.Field)(i + 0x01)]);   // restore text of those GUI input fields that user had entered
                            return;
                        default:
                            SetStatement(GetMsg(string.Empty + (char)0x0412 + (char)0x043E + (char)0x0437 + (char)0x043D + (char)0x0438 + (char)0x043A + (char)0x043B + (char)0x0438 + (char)0x0020 + (char)0x043D + (char)0x0435 + (char)0x043A +
        ...
        private void button1_Enter(object sender, EventArgs e)
        {
            ...
            {
                case true:
                    for (var i = (byte)default; i < custContainer.Length; i++)
                        custContainer[i] = string.Empty;
                    return;
            }
        }
        ...
        {
            CLS();
            SetFieldColour(SystemColors.Window);
            SetGUI(true, act);
        }
                ...
                case true:
                    switch (hintSign ^= true)
                    {
                        case false:   // off
                            ...
                            return;
                        default:   // on
                            SetFieldsHint(Convert.ToByte(custContainer[default].Split(separator).Length - 0x01), Color.Lime);   // amount of intended GUI input fields as indexes. Colour supposes to provide the hint
                            return;
                    }
            }
        }
        ...
        {
            for (var i = (byte)default; i < capacity; i++)
            {
                hint?.Invoke(Convert.ToByte(custContainer[default].Split(separator)[i]), colour);
            }
        }
        ...
        private void SetStatement(in string ask)   // scene switching
        {
        ...
        private DataTable ExecSQLtoSet(in string sqlQuery)
        {
            using (var dbConn = GetConnect())
            using (var datAdapter = new OleDbDataAdapter(sqlQuery, dbConn))
            {
                var datSet = new DataSet();   // a store
                datAdapter.Fill(datSet);   // copy cached data to the store 
                return datSet.Tables[(int)default];   // resulting table
            }
        }
                ...
                using (var cmdR = cmd.ExecuteReader() as OleDbDataReader)
                    do
                        while (cmdR.Read())   // skim data
                            container.Enqueue(cmdR[(int)default]);   // put data(resulting column) into container
        ...
        private void ExecSQLPlural(params string[] sqlQuery)
        {
            using (var dbConn = GetConnect())
            {
                ...
                {
                    for (var i = (byte)default; i < sqlQuery.Length; i++)
                    {
                        cmd.CommandText = sqlQuery[i];   // enclose more, queries
                        cmd.ExecuteNonQuery();
                    }
        ...
        private void SetFieldsTextColour(in Color colour)
        {
            textBox1.ForeColor = colour;
            label1.ForeColor = colour;
            textBox2.ForeColor = colour;
            textBox3.ForeColor = colour;
            textBox4.ForeColor = colour;
            textBox5.ForeColor = colour;
            textBox6.ForeColor = colour;
            label2.ForeColor = colour;
            label3.ForeColor = colour;
            label4.ForeColor = colour;
        }
        ...
        private unsafe string GetTextFormatted(in char[] text, in char[] symb)
        {
            ...
            {
                bool isSymb = default;
                for (var i = (byte)default; i < text.Length; i++, isSymb = default)
                    for (var j = (byte)default; j < symb.Length; j++)
        ...
        private void SetFieldColour(in Color colour)
        {
        ...
        private void SetFieldColour(in byte index, in object sender)
        {
            switch (index)
            {
                case 0x00:
        ...
        {
            switch (container.ToString().Split(separator)[index].Length > byte.Parse(condition[index].ToString()))   // deal with length of GUI input field
            {
                case true:
            ...
            switch (condition)
            {
                ...
                default:
                    for (var i = indStart; i < indEnd; i++)
                        switch (sender[i].ToString().Length > byte.Parse(condition[i].ToString()))   // deal with length of GUI input field
                        {
                            case false:
                                SetFieldColour(i, Color.Salmon);   // incorrect GUI input field
        ...
        {
            var tabWarnPatient = new char[20] { (char)0x0411, (char)0x0435, (char)0x0437, (char)0x0020, (char)0x0434, (char)0x0430, (char)0x043D, (char)0x043D, (char)0x044B, (char)0x0445, (char)0x0020, (char)0x043F, (char)0x0430, (char)0x0446, (char)0x0438, (char)0x0435, (char)0x043D, (char)0x0442, (char)0x0430, (char)0x003F };
            var tabWarnVaccine = new char[25] { (char)0x0411, (char)0x0435, (char)0x0437, (char)0x0020, (char)0x0438, (char)0x043D, (char)0x0444, (char)0x043E, (char)0x0440, (char)0x043C, (char)0x0430, (char)0x0446, (char)0x0438, (char)0x0438, (char)0x0020, (char)0x043E, (char)0x0020, (char)0x0432, (char)0x0430, (char)0x043A, (char)0x0446, (char)0x0438, (char)0x043D, (char)0x0435, (char)0x003F };
            var summary = new byte[0x03]
            {
                default,   // reserved for range of all GUI input fields
                default,   // reserved for range of Пациент input fields
                default    // reserved for range of Вакцина input fields
            };
            for (var i = (byte)default; i < sender.Length; i++)
                SetFieldsSum(sender, i, ref summary);
            onset = (byte)name.Field.ПлановаяДата;   // on init: amount of all GUI input fields
            switch (act)
            {
                case 0x01:
                    switch (sender.Length == 0x0A)   // does it equal to sent GUI input fields
                    {
                        case true:   // sent GUI input fields
                            ...
                            return ConfineFields((byte)name.Field.ФИО - 0x01, onset, sender, condition);   // determine all GUI input fields, in order
                    }
                    return default;
                case 0x02:
                    ...
                    {
                        case true:
                            SetFieldColour(summary[default], Color.Salmon);   // useless ConfineFields(onset = (byte)name.Field.ФИО - 0x01, ++summary[default], sender, condition) might be used to optimize the code
                            return default;
                        default:
                ...
                case 0x03:
                    switch (sender.Length == 0x0A)   // does it equal to sent GUI input fields
                    {
                        case true:   // sent GUI input fields
                            ...
                            return true;   // let GUI input fields be empty. Useless ConfineFields(0x00, 0x00, sender, condition) might be used to optimize the code
                    }
                    return default;
        ...
        {
            SetFieldColour(default, SystemColors.Window);
            toolStripStatusLabel1.Text = GetTextFormatted(reg.Tab.Patient.tagSlf, nil.ToString().ToCharArray());
        }
        ...
        {
            SetFieldColour(0x01, SystemColors.Window);
        ...
        {
            toolStripStatusLabel1.Text = GetTextFormatted(reg.Tab.Patient.tagHomeAddr, nil.ToString().ToCharArray());
            SetFieldColour(0x02, SystemColors.Window);
        }
        ...
        {
            toolStripStatusLabel1.Text = GetTextFormatted(reg.Tab.Patient.tagCompany, nil.ToString().ToCharArray());
            SetFieldColour(0x03, SystemColors.Window);
        }
        ...
        {
            toolStripStatusLabel1.Text = GetTextFormatted(reg.Tab.Patient.tagOccupation, nil.ToString().ToCharArray());
            SetFieldColour(0x04, SystemColors.Window);
        }
        ...
        {
            toolStripStatusLabel1.Text = GetTextFormatted(reg.Tab.Vaccine.tagName, nil.ToString().ToCharArray());
            SetFieldColour(0x05, SystemColors.Window);
        }
        ...
        {
            toolStripStatusLabel1.Text = GetTextFormatted(reg.Tab.Vaccine.tagSerial, nil.ToString().ToCharArray());
            SetFieldColour(0x06, SystemColors.Window);
        }
        private void comboBox1_TextChanged(object sender, EventArgs e)
        {
            label2.Text = comboBox1.Text;   // on init: to measure GUI input length of the further event
            SetFieldColour(0x07, SystemColors.Window);
        }
        ...
        {
            SetFieldColour(0x08, SystemColors.Window);
        ...
        {
            SetFieldColour(0x09, SystemColors.Window);
            switch (button1.Text == ops.Action.Записать.ToString())
            {
                case true:
                    return;
